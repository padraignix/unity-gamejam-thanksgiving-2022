			...
			for(int a = 0; a < shupUi.Length; a++){
				if(a + cPage < shopSlot.Length && shopSlot[a + cPage].itemId > 0){
					if(shopSlot[a + cPage].itemType == ItType.Usable){
						...
						shupUi[a].descriptionText.text = database.usableItem[shopSlot[a + cPage].itemId].description;
						...
						shupUi[a].descriptionText.text = database.equipment[shopSlot[a + cPage].itemId].description;
		...
		if(mode == 1){
			...
			for(int a = 0; a < shupUi.Length; a++){
				if(a + cPage < player.GetComponent<InventoryC>().itemSlot.Length && player.GetComponent<InventoryC>().itemSlot[a + cPage] > 0){
					...
					shupUi[a].descriptionText.text = database.usableItem[player.GetComponent<InventoryC>().itemSlot[a + cPage]].description;
		...
		if(mode == 2){
			...
			for(int a = 0; a < shupUi.Length; a++){
				if(a + cPage < player.GetComponent<InventoryC>().equipment.Length && player.GetComponent<InventoryC>().equipment[a + cPage] > 0){
					...
					shupUi[a].descriptionText.text = database.equipment[player.GetComponent<InventoryC>().equipment[a + cPage]].description;
