...
public class QuestUiCanvasC : MonoBehaviour {
	...
	public void UpdateQuestDetails(){
		...
		for(int a = 0; a < questName.Length; a++){
			...
			questDescription[a].GetComponent<Text>().text = db.questData[pq.questSlot[a]].description + " (" + QuestStatC.questProgress[pq.questSlot[a]].ToString() + " / " + db.questData[pq.questSlot[a]].finishProgress + ")";
			if(pq.questSlot[a] > 0){
	...
	public void CancelQuest(int qid){
		...
		QuestStatC pq = player.GetComponent<QuestStatC>();
		QuestStatC.questProgress[pq.questSlot[qid]] = 0;
		pq.questSlot[qid] = 0;
		pq.SortQuest();
		UpdateQuestDetails();
	}
