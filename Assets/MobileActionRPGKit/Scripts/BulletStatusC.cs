...
public class BulletStatusC : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other) {  	
		...
		if (shooterTag == "Player" && other.tag == "Enemy") {
			dmgPop = (Transform)Instantiate(Popup, other.transform.position , transform.rotation);
			...
			dmgPop.GetComponent<DamagePopupC>().damage = popDamage.ToString();	
			...
			if (flinch) {
				...
				other.GetComponent<StatusC>().Flinch(dir);
			}
			...
			dmgPop = (Transform)Instantiate(Popup, transform.position , transform.rotation);
